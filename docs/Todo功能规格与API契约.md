...
| 中栏 | 浮动表面卡片（玻璃渐变 + 圆角 12 + 投影）：视图标题 / 计数、搜索框、排序、多选、快速添加输入框、任务列表、已完成折叠按钮、全局进度 | 自适应 |
...
### 5.2.1 搜索（`searchNeedle()` / `matchesSearch()`）

- 位置：中栏 `head` 行，排序下拉左侧；`QLineEdit#TodoSearch`，placeholder `搜索任务…`，带清空按钮，宽 `si(200)` 高 `si(28)`
- 口径：**纯内存过滤，不走服务端**（`ApiClient::getTodos` 无 search 参数，任务全量已缓存在 `m_tasks`）
  - 匹配字段：`title` / `notes` / `tags` 每一项 / **所属清单名**（`listId` 查 `m_lists`）
  - 匹配方式：`trim().toLower()` 后**子串包含**，不区分大小写，多词不做分词
  - 生效范围：先按视图（5.2）筛，再叠加搜索；空词 = 不过滤
- 交互：`textChanged` → 200ms 防抖 → `rebuildList()`（与笔记页搜索同一手感）
- 平铺（看板）视图：看板列直接吃 `m_tasks` 而非 `visibleTasks()`，故 `rebuildBoard()` 内对副本单独过一遍
  `matchesSearch`，计数 `N 项待办` 同样按过滤后的集合算
