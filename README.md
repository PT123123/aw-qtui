...
任务管理器「内存」列是**工作集**，里面含映射进来的字体文件 —— 实测光系统字体库（DirectWrite）
一次性初始化就要 **+30 MB 私有 / +69 MB 工作集**。所以一个空壳 Qt Widgets 窗口在任务管理器里
就显示 130 MB，这跟我们的代码无关。**评估开销一律看私有内存（任务管理器「提交大小」列）**。

### 地板实测（`just mem-baseline`）

真实 windows 平台、不弹窗、空数据、不连服务端（2026-09-23；Qt 6.8.3 + MSVC 19.44，
两次运行结果一致。同机偶发 ±10 MB 抖动来自原生窗口创建时机，所以看「增量」比看绝对值稳）：

| 阶段 | 私有 | Δ私有 | 工作集 |
|---|---|---|---|
| QApplication 起来 | 4.2 | — | 24.6 |
| + 空窗口 1200×800 show | 9.2 | +5.0 | 33.5 |
| + 全局 QSS | 9.5 | +0.3 | 34.3 |
| + 首帧文字（逼出系统字体库） | 40.0 | **+30.5** | 103.0 |
| + InboxPage（空数据，已 show+polish） | 47.7 | +7.7 | 114.1 |
| + TodoPage（空数据） | 62.4 | **+14.7** | 130.2 |

**空数据地板 ≈ 62 MB 私有 / 130 MB 工作集**：其中 30 MB 是系统字体库、不算我们的开销，
剩下 ~32 MB 才是 Qt 本体 + 两个常驻页的空壳。

> 读数注意：真机第一次画文字才建字体缓存，所以第 3b 级必须单独量出来，否则这 30 MB 会被
> 记到下一个页面头上。离屏平台（`just mem-baseline offscreen`）没有这一级，绝对值也低得多
> （同一份代码：地板 25.4 MB 私有 / 49.1 MB 工作集），只适合看每级增量、以及快速回归。

### 预算（超了按超标处理）

| 场景 | 判据（私有内存） |
|---|---|
| 冷启动空闲（收件箱 / 任务页已建，未进过容器页） | ≤ 130 MB |
| 真实数据常态（几百条笔记 + 百余任务） | ≤ 160 MB |
| 快速切页压力（笔记 ↔ 任务来回 20 次） | 增量 ≤ 15 MB，停手 30 s 内回落 |
| 切走容器页（活动/专注/同步/设置）5 s 后 | 应回落（懒建页整树销毁） |

2026-09-23（改进前）实测，本机数据 278 条笔记 + 117 条任务：私有 **245.6 MB**、工作集 261 MB、
峰值 263 / 340 MB，且空闲 5 分钟只回落 3.5 MB —— 典型「高水位顶上去不还」。超标主因是
**每次切页都整表重建**（下节），以及全仓每控件 `setStyleSheet` 的样式开销。

同机同数据，**0.1.24（切页代次门控 + 快速连切跳过淡入）冷启动后空闲 2 分钟采样**：
私有 **68.1 ~ 71.5 MB**、工作集 147.2 ~ 150.3 MB、峰值私有 **77.3 MB**（仍在预算内，且采样期间
单调微降、没有爬升趋势）。

### 不要破坏的约定

- **`MainWindow::applyUiScale()` 的语义是「只为缩放/主题变化重建」**。它被 `switchPage()` 调用，
  但现在有 `m_scaleEpoch` / `m_pageScaleEpoch` 代次门控：平切（缩放和主题都没变）直接返回，
  零重建。新增「每次进入页面都要做一次」的逻辑，请挂到数据变化（`dataChanged`）、`refresh()`
  或 `showEvent` 上，**别加进 `applyUiScale()`** —— 加进去就等于每次切页全量重建一次。
- 页面级 `applyUiScale()` 是重活：收件箱结尾是 `applyClientFilter(true)`（`force=true` 让
  `renderSignature` 去重失效 → 整表重建），任务页是 `rebuildNavLists() + updateNavCounts() +
  rebuildList()` 三连。**一次调用 = 几百个控件的构造/析构**，调用点必须克制。
- 可淘汰页（活动/专注/同步/设置）离开即销毁子控件树、回来才懒建，所以它们的 `applyUiScale()`
  在每次进入时照旧要跑（`switchPage()` 里显式作废这些页的代次）。
- 整页 `opacity` 淡入每帧都要离屏渲染整页，代价随卡片数线性增长；`switchPage()` 里距上次切页
  不足 `kRapidSwitchMs`（220ms）会跳过淡入，避免快速连切时 CPU/内存一起冲高。
- Qt 给**每个自带样式表的控件**单独建一份 `QStyleSheetStyle` 和解析好的规则表，控件之间不共享 ——
  能进全局 QSS 的走 objectName 选择器，不要逐控件 `setStyleSheet`。
- 自查：`grep -c 'scaleCurrentView'` 的每个新增调用点都该问一句「这里真的需要重建吗」。

### 尚未收敛（超出当前范围，按需推进）

- 全仓约 279 处 `setStyleSheet`（24 个文件）→ 可迁移为全局 QSS 的 objectName 选择器；
- `CardPool(60)` 常驻 60 张卡的整棵控件树；
- 离线时 `renderLocal()` 全量入列（在线为分页 20 条/页）；
- 空 TodoPage 单项 +14.7 MB 偏大（详情栏常驻编辑器 + 每控件样式表）。

### 复现

```sh
just build Release build-verify   # 改过 src/ 必须先跑：探针复用这份目标文件（否则量到旧代码）
just mem-baseline                 # real 平台（默认，不弹窗，最接近实机）
just mem-baseline offscreen       # 离屏平台：快，但绝对值偏低，只看每级增量
```

实机常态另有不看窗口的采法：读 `%APPDATA%\aw-qtui\aw-qtui\awqtui.instance.json` 里的 `pid`，
再取 `tasklist /FI "PID eq <pid>"` 的「提交大小」列（即私有内存）。
